...
@Component
@Slf4j
public class RSIBearishBacktest implements StrategyBacktest {
    ...
    @Autowired
    public RSIBearishBacktest(HistoricalDataService historicalDataService, CriterionsFactory criterionsFactory) {
    ...
    public Class<? extends TradeStrategy> strategy() {
        return RSIBearishStrategy.class;
    }
        ...
        final Function<Variables<Parameter>, TradeStrategy> recipe = var -> {
            backtestParameters.forEach((key, value) ->
                Variables.addValues(var,
                    ...
                    value));
            return RSIBearishStrategy.build(context, var.getParameterValues());
        };
    ...
    private Function<TradeScenario, BacktestResult> gatherResult(List<String> analysisCriterions) {
        return tradeScenario -> {
            var criterions = analysisCriterions.stream()
